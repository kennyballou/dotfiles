...
local fs = {
    ...
    docker  = awful.widget.progressbar(),
...
vicious.register(fs.docker, vicious.widgets.fs, "${/var/lib/docker used_p}", 599)
...
vicious.register(
    ...
    beautiful.fg_netup_widget .. '">${eth0 up_kb}</span> <span color="' ..
    beautiful.fg_netdn_widget .. '">${eth0 down_mb}</span>',
    3
)
...
generate_layout = function(s, taglist, tasklist, promptbox)
    ...
    top_left_layout:add(fs.docker)
    top_left_layout:add(fs.usr)
